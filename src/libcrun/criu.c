...
#  endif
  ...
  /* Tell CRIU the container's actual cgroup path so it can rewrite the
     checkpoint image's cgroup entries before restore.  Without this,
     the restored process's cgroup namespace root points to the OLD
     cgroup scope from the checkpoint, causing re-checkpoint to fail
     with -EBADE (cgroup path suffix mismatch in proc_parse.c).  */
  if (status->cgroup_path && libcriu_wrapper->criu_add_cg_root)
    {
